...
public class ArraysV3 {
	...
	public static void main(String[] args) {
		 ...
		 * for() 명령문의 index < 100 을 index < 200으로 변경해 주어야 한다.
		 * 만약 for() 명령문의 코드를 변경하지 않으면
		 * 테스트 데이터의 개수를 늘린 효과가 없어진다.
		 * 
		 * 배열의 개수를 변경할때만다 코드를 추가로 변경하는 불편을 없애기 위해
		 * 자바에서는 배열의 크기와 관련된 내부 속성을 마련해 두었다.
		 * 배열.length 속성은 배열의 개수를 담고있는 특별한 속성이다.
		 * 배열.length 는 배열을 생성하면 배열의 개수를 보관하도록 자동으로 만들어지는 특별한 변수이다.
		 * 
		 * 참고로 배열.toString() : 배열의 메서드 
		 * 배열.length : 배열의 속성
		 */
		...
		intSum = 0;
		// intScore 배열의 0번 부터 inScore.length -1 번까지의
		// 점수를 모두 합산하여 intSum에 담고, console 에 출력
		for (index = 0; index < intScore.length; index++) {
		...
		// 평균 결과를 소수점까지 표현하고 싶으면 원 데이터를 먼저 실수형으로 강제 형변환 한 후에
